...
function patientInfoTextHC(p,coverage){
  ...
  function openEvolutionModalHC(key,evolutionId='',atencionId=''){
    ...
    const episodeContextHtml410e=attention?`<div class="hc-episode-context411e"><span class="hc-episode-context-title411e">Contexto del turno · solo lectura</span><div class="hc-episode-context-grid411e"><div><span>Derivante</span><strong>${escHC(attention.derivante||'s/d')}</strong></div><div><span>Motivo</span><strong>${escHC(attention.motivoConsulta||'s/d')}</strong></div>${attention.observacionClinica?`<div class="full"><span>Observación clínica</span><strong>${escHC(attention.observacionClinica)}</strong></div>`:''}</div></div>`:'';
